...
- **Widgety** — živé dlaždice, ktoré sa kreslia z dát a času, bez API. **Hodiny**: čas, dátum a číslo ISO týždňa s paritou párny/nepárny (nastavenia: 12/24 h, sekundy, dátum, týždeň). **Šprint**: 14-dňový kalendárny pásik ukazujúci, kde sa tím práve nachádza v dvojtýždňovom šprinte — jeden widget na jeden tím, takže dva tímy striedajúce sa v párnych/nepárnych týždňoch sú dva widgety. Každý widget má vlastný dialóg nastavení (✎ v režime úprav); šprintový nastavuje názov tímu, či šprinty začínajú v párnom alebo nepárnom ISO týždni, a farbu tímu (5 prednastavených + vlastná paleta). Widgety sa presúvajú, rozťahujú a patria plochám ako poznámky, synchronizujú sa cez `links.js`.
